...
std::vector<AuditEntry> AuditService::events_for_task(const std::string &task_id)
{
    ...
    {
        entries.push_back(
            AuditEntry{value_or_empty(row, "id"), value_or_empty(row, "type"),
                       value_or_empty(row, "task_id"), value_or_empty(row, "workflow_id"),
                       value_or_empty(row, "before_state"), value_or_empty(row, "after_state"),
                       value_or_empty(row, "reason"), value_or_empty(row, "occurred_at")});
    }
    return entries;
}
                                          ...
                                          const std::string &after_state)
{
    ...
    sql << "INSERT INTO audit_events(id, type, task_id, workflow_id, actor, before_state, "
        ...
        << ", 'SystemRecovered', " << quote(task_id) << ", " << quote(workflow_id) << ", 'system', "
        << quote(before_state) << ", " << quote(after_state)
